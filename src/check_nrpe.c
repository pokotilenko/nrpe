...
int main(int argc, char **argv){
	...
	if(result==STATE_OK){
		...
		/* check packet type */
		if(ntohs(receive_packet.packet_type)!=RESPONSE_PACKET && ntohs(receive_packet.packet_type)!=RESPONSE_PACKET_PART){
		...
		} while (ntohs(receive_packet.packet_type)==RESPONSE_PACKET_PART);
